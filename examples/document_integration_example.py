...
def main():
    ...
    print(f"✓ TextExtractor initialized with whitespace normalization")
    ...
    print("-" * 40)
    mapper = AnchorMapper()
    print(f"✓ AnchorMapper initialized")
    ...
    detections = analyzer.analyze(text=full_text, entities=["PHONE_NUMBER", "EMAIL_ADDRESS", "PERSON"])
    
    # Step 4: Create anchors for detected entities
    mapper = AnchorMapper()
    original_texts = {seg.node_id: seg.text for seg in segments}
    anchors = mapper.create_anchors_from_detections(detections, segments, original_texts)
    ...
    print(f"\nProcessing stats: {processor.get_processing_stats().files_processed} files processed")

...
class TestSurrogateStrategy:
    ...
    def test_surrogate_replaces_with_fake_data_not_asterisks(self):
        ...
        policy = MaskingPolicy(
            default_strategy=Strategy(
                kind=StrategyKind.SURROGATE, parameters={"seed": "test-seed"}
            ),
            seed="test-seed",
        )
    ...
    def test_surrogate_deterministic_with_seed(self):
        ...
        policy = MaskingPolicy(
            default_strategy=Strategy(
                kind=StrategyKind.SURROGATE, parameters={"seed": "consistent-seed"}
            ),
    ...
    def test_surrogate_different_entity_types(self):
        ...
        policy = MaskingPolicy(
            default_strategy=Strategy(kind=StrategyKind.SURROGATE, parameters={"seed": "test"})
        )
        engine = CloakEngine(default_policy=policy)
        ...
        result = engine.mask_document(
            doc, entities=["PERSON", "EMAIL_ADDRESS", "PHONE_NUMBER", "DATE_TIME", "LOCATION"]
        )
    ...
    def test_surrogate_with_multiple_same_entities(self):
        ...
        policy = MaskingPolicy(
            default_strategy=Strategy(
                kind=StrategyKind.SURROGATE, parameters={"seed": "consistency-test"}
            )
        )
        engine = CloakEngine(default_policy=policy)
    ...
    def test_surrogate_empty_parameters(self):
        ...
        policy = MaskingPolicy(
            default_strategy=Strategy(kind=StrategyKind.SURROGATE, parameters={})
        )
        engine = CloakEngine(default_policy=policy)
    ...
    @pytest.mark.parametrize(
        "entity_type,original,should_have",
        [
    ...
    def test_surrogate_entity_specific_format(self, entity_type, original, should_have):
        ...
        policy = MaskingPolicy(
            default_strategy=Strategy(
                kind=StrategyKind.SURROGATE, parameters={"seed": f"test-{entity_type}"}
            )
        )
        engine = CloakEngine(default_policy=policy)

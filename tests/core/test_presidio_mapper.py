...
class TestStrategyToOperatorMapper:
    ...
    def test_redact_strategy_custom_char(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_redact_strategy_preserve_length(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_template_strategy_auto_generate(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_hash_strategy_custom_algorithm(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_hash_strategy_with_salt(self, mapper: StrategyToOperatorMapper) -> None:
        ...
        strategy = Strategy(
            StrategyKind.HASH, {"algorithm": "sha256", "salt": "my-salt"}
        )
        result = mapper.strategy_to_operator(strategy)
    ...
    def test_hash_strategy_per_entity_salt(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_partial_strategy_basic(self, mapper: StrategyToOperatorMapper) -> None:
        ...
        assert result.params["from_end"]
        assert result.params["masking_char"] == "*"

    def test_partial_strategy_start_position(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
        """Test PARTIAL strategy with start position."""
        strategy = Strategy(
            StrategyKind.PARTIAL,
            {"visible_chars": 3, "position": "start", "mask_char": "#"},
        )
        result = mapper.strategy_to_operator(strategy)

        assert result.operator_name == "mask"
        assert result.params["chars_to_mask"] == -1
        assert not result.params["from_end"]
        assert result.params["masking_char"] == "#"

    def test_partial_strategy_end_position(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
        """Test PARTIAL strategy with end position."""
        strategy = Strategy(
            StrategyKind.PARTIAL,
            {"visible_chars": 4, "position": "end", "mask_char": "X"},
        )
        result = mapper.strategy_to_operator(strategy)
    ...
    def test_partial_strategy_unsupported_positions(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
        """Test PARTIAL strategy with unsupported positions."""
        for position in ["middle", "random"]:
            strategy = Strategy(
                StrategyKind.PARTIAL, {"visible_chars": 2, "position": position}
            )
            result = mapper.strategy_to_operator(strategy)
    ...
    def test_surrogate_strategy_dictionary(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_custom_strategy_valid_callback(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_custom_strategy_missing_callback(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_custom_strategy_invalid_callback(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_policy_to_operators_basic(self, mapper: StrategyToOperatorMapper) -> None:
        ...
        assert not result["EMAIL_ADDRESS"].params["from_end"]

    def test_policy_to_operators_empty_policy(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_policy_to_operators_with_failures(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
        ...
        policy = MaskingPolicy(
            per_entity={
                ...
                "BAD_ENTITY": Strategy(
                    StrategyKind.CUSTOM, {"callback": good_callback}
                ),
            }
        )
    ...
    def test_edge_case_invalid_strategy_kind(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_mapping_exception_handling(
        self, mapper: StrategyToOperatorMapper, monkeypatch
    ) -> None:
    ...
    def test_all_strategy_kinds_supported(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_parameter_validation_preserved(
        self, mapper: StrategyToOperatorMapper
    ) -> None:
    ...
    def test_complex_policy_scenario(self, mapper: StrategyToOperatorMapper) -> None:
        ...
        policy = MaskingPolicy(
            ...
            per_entity={
                "PHONE_NUMBER": Strategy(
                    StrategyKind.PARTIAL,
                    {"visible_chars": 4, "position": "end", "mask_char": "*"},
                ),
                "EMAIL_ADDRESS": Strategy(
                    StrategyKind.HASH, {"algorithm": "sha256", "salt": "email_salt"}
                ),

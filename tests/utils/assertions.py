...
def assert_cloakmap_valid(cloakmap: CloakMap) -> None:
    ...
    for anchor in cloakmap.anchors:
        ...
        assert hasattr(anchor, 'masked_value'), f"Anchor should have masked_value: {anchor}"
        assert hasattr(anchor, 'entity_type'), f"Anchor should have entity_type: {anchor}"
...
def assert_policy_applied_correctly(
    ...
    for entity in entities:
        ...
        if entity.score >= threshold and policy.should_mask_entity(
            ...
            matching_anchors = [
                anchor for anchor in result.cloakmap.anchors

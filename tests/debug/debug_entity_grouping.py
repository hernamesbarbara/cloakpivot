...
entities_info = [
    {"type": "PERSON", "text": "Cameron MacIntyre", "start": 6, "end": 23, "score": 0.85},
    {"type": "EMAIL_ADDRESS", "text": "cameron@example.com", "start": 25, "end": 44, "score": 1.00},
...
for e in entities_info:
    print(f"{e['type']}: [{e['start']}, {e['end']}) = '{e['text']}' (score: {e['score']})")
...
print(f"Config: strategy={config.strategy.value}, preserve_high_confidence={config.preserve_high_confidence}")
...
for detail in result.resolution_details:
    print(f"  Group {detail['group_index']}: {detail['original_count']} -> {detail['resolved_count']}")

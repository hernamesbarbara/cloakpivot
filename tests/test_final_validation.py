...
class TestPerformanceValidation:
    ...
    def test_singleton_pattern_effectiveness(self):
    ...
    def test_session_fixture_performance(self):
    ...
    def test_entity_detection_performance(self):
    ...
    def test_validation_script_execution(self):
        ...
        pytest_running = (
            ...
            any('pytest' in str(frame.filename) for frame in sys._current_frames().values())
        )
        if pytest_running:
        ...
        try:
            ...
            result = subprocess.run([
                ...
                "--output", "/tmp/test_validation_report.md",
                "--json-output", "/tmp/test_validation_results.json",
                "--verbose"
            ...
            assert Path("/tmp/test_validation_report.md").exists(), "Validation report should be created"
            assert Path("/tmp/test_validation_results.json").exists(), "JSON results should be created"
            
            # Verify JSON output structure
            with open("/tmp/test_validation_results.json") as f:
            ...
            try:
                ...
                os.unlink("/tmp/test_validation_report.md")
                os.unlink("/tmp/test_validation_results.json")
            except FileNotFoundError:
                pass
    
    def test_performance_target_calculations(self):
        ...
        assert reduction_target.improvement_pct == 60.0, f"Expected 60% improvement, got {reduction_target.improvement_pct}"
...
class TestErrorHandling:
    ...
    def test_invalid_baseline_format_handling(self):
    ...
    def test_test_execution_failure_handling(self, mock_run):

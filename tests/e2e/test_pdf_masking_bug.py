...
class TestPDFMaskingBug:
    ...
    def test_minimal_masking_case(self):
        ...
        for i, text in enumerate(test_texts):
            ...
            for entity in entities:
                ...
                print(
                    f"  Detected: {entity.entity_type} = '{entity_text}' (score: {entity.score:.2f})"
                )
        ...
        for detection in person_detections:
            print(
                f"  - '{detection['entity_text']}' in text {detection['text_index']} (score: {detection['score']:.2f})"
            )
    ...
    def test_full_pdf_masking_workflow(self):
        ...
        person_anchors = [
            a for a in mask_result.cloakmap.anchors if a.entity_type == "PERSON"
        ]
        ...
        subject_line_original = (
            original_texts[3].text if len(original_texts) > 3 else ""
        )

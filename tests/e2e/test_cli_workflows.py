...
Email: jane.smith@personal.com"""
    ...
    def test_complete_mask_unmask_workflow(
        ...
        assert original_content.strip() == unmasked_content.strip(), "Round-trip should preserve content"
...
"""
        ...
        # Test batch masking
        batch_result = cli_runner.invoke(cli, [
            "batch", "mask",
            "--input-dir", str(input_dir),
            "--output-dir", str(output_dir),
            "--cloakmap-dir", str(cloakmap_dir),
            "--policy", str(sample_policy_file)
        ])
    ...
    def test_error_handling_workflows(
        ...
        result = cli_runner.invoke(cli, [
            "mask",
            "--input", str(temp_workspace / "nonexistent.txt"),
            "--output", str(temp_workspace / "output.txt"),
            "--policy", str(temp_workspace / "policy.json")
        ])
    ...
    def test_output_format_workflow(
        ...
        result = cli_runner.invoke(cli, [
            "mask",
            "--input", str(sample_document_file),
    ...
    def test_verbose_and_quiet_modes(
        ...
        # Test verbose mode
        verbose_result = cli_runner.invoke(cli, [
            "mask",
            "--input", str(sample_document_file),
        ...
        # Test quiet mode
        quiet_result = cli_runner.invoke(cli, [
            "mask",
            "--input", str(sample_document_file),
            "--output", str(output_file),
            "--policy", str(sample_policy_file),
    ...
    def test_plugin_workflow(
        ...
        if result.exit_code == 0:
            ...
            assert "strategy" in result.output.lower() or "recognizer" in result.output.lower()
        else:
    ...
    def test_validation_workflow(
        ...
        policy_result = cli_runner.invoke(cli, [
            "policy", "validate", "--policy", str(sample_policy_file)
        ])
    ...
    def test_cli_integration_with_temp_files(
        ...
        test_content = "Test document with phone 555-123-4567 and email test@example.com"

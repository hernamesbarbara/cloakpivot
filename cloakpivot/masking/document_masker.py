...
class DocumentMasker:
    ...
    def _is_text_bearing_node(self, node_item: NodeItem) -> bool:
        ...
        return isinstance(node_item, (
            TextItem, TitleItem, SectionHeaderItem, 
            ListItem, CodeItem, FormulaItem
        ))
    
    def _mask_text_node(
        self, 
        node_item: Union[TextItem, TitleItem, SectionHeaderItem, ListItem, CodeItem, FormulaItem],
        anchors: List[AnchorEntry]
    ) -> None:
    ...
    def _mask_table_node(
        ...
        for cell_node_id, cell_anchor_list in cell_anchors.items():
            ...
            # Check bounds
            if (row_idx >= len(table_data.table_cells) or 
                col_idx >= len(table_data.table_cells[row_idx])):
                logger.warning(
            ...
            if hasattr(cell, "text") and cell.text:
                ...
                sorted_anchors = sorted(cell_anchor_list, key=lambda a: a.start, reverse=True)
    ...
    def _mask_key_value_node(
        ...
        # Mask value part
        if value_anchors and hasattr(kv_item, "value") and kv_item.value and hasattr(kv_item.value, "text"):

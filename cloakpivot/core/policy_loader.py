...
class StrategyConfig(BaseModel):
    ...
    def validate_kind(cls, v):
        ...
        try:
            ...
            raise ValueError(f"Invalid strategy kind '{v}'. Valid kinds: {valid_kinds}")
        return v
...
class EntityConfig(BaseModel):
    ...
    enabled: Optional[bool] = Field(None, description="Whether entity recognition is enabled")
...
class LocaleConfig(BaseModel):
    ...
    recognizers: Optional[List[str]] = Field(None, description="Custom recognizers for locale")
    entity_overrides: Optional[Dict[str, EntityConfig]] = Field(None, description="Entity overrides for locale")


class ContextRuleConfig(BaseModel):
    """Pydantic model for context-specific rules."""
    enabled: Optional[bool] = Field(None, description="Whether context is enabled for masking")
    strategy_overrides: Optional[Dict[str, EntityConfig]] = Field(None, description="Strategy overrides for context")
    threshold_overrides: Optional[Dict[str, float]] = Field(None, description="Threshold overrides for context")
...
class PolicyFileSchema(BaseModel):
    ...
    extends: Optional[Union[str, List[str]]] = Field(None, description="Base policy files to inherit from")
    ...
    default_strategy: Optional[StrategyConfig] = Field(None, description="Default masking strategy")
    per_entity: Optional[Dict[str, EntityConfig]] = Field(None, description="Per-entity configurations")
    
    # Thresholds and filtering
    thresholds: Optional[Dict[str, float]] = Field(None, description="Per-entity confidence thresholds")
    allow_list: Optional[List[Union[str, AllowListItem]]] = Field(None, description="Values to never mask")
    deny_list: Optional[List[str]] = Field(None, description="Values to always mask")
    min_entity_length: Optional[int] = Field(1, ge=0, description="Minimum entity length to consider")
    
    # Locale support
    locales: Optional[Dict[str, LocaleConfig]] = Field(None, description="Locale-specific configurations")
    
    # Context rules
    context_rules: Optional[Dict[str, ContextRuleConfig]] = Field(None, description="Context-specific rules")
    
    # Policy composition
    policy_composition: Optional[PolicyCompositionConfig] = Field(None, description="Policy composition settings")
...
class PolicyLoader:
    ...
    def _load_policy_file(self, policy_path: Path, context: PolicyLoadContext) -> PolicyFileSchema:
        """Load and resolve a single policy file with inheritance."""
        ...
        if policy_schema.extends:
            ...
            base_policies = []
            extends_list = policy_schema.extends if isinstance(policy_schema.extends, list) else [policy_schema.extends]
    ...
    def _merge_policies(self, policies: List[PolicyFileSchema]) -> PolicyFileSchema:
        """Merge multiple policy schemas using composition rules."""
        if not policies:
    ...
    def _merge_two_policies(self, base: PolicyFileSchema, override: PolicyFileSchema) -> PolicyFileSchema:
        ...
        if composition_config.merge_strategy == "override":
            ...
            for key, value in override_data.items():
                if value is not None:
                    ...
                    elif key in ["thresholds", "locales", "context_rules"] and isinstance(value, dict):
            ...
            allowed_override_fields = {"name", "description", "per_entity", "thresholds", "allow_list", "deny_list"}
            for key, value in override_data.items():

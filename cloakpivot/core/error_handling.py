...
class FailureToleranceConfig:
    ...
    max_consecutive_failures: int = 5  # Maximum consecutive failures before stopping
...
class ErrorCollector:
    ...
    def record_success(self, context: Optional[Dict[str, Any]] = None) -> None:
        ...
        logger.debug(f"Operation succeeded (total: {self.total_operations}, successes: {self.success_count})")
...
class PartialFailureManager:
    ...
    def should_continue_processing(self) -> bool:
        ...
        if failure_rate > self.tolerance_config.max_failure_rate:
            logger.warning(f"Failure rate {failure_rate:.2%} exceeds threshold {self.tolerance_config.max_failure_rate:.2%}")
            return False
        ...
        if consecutive >= self.tolerance_config.max_consecutive_failures:
            logger.warning(f"Consecutive failures {consecutive} exceeds threshold {self.tolerance_config.max_consecutive_failures}")
            return False
        ...
        if (
            ...
            logger.warning(f"Success count {self.error_collector.success_count} below minimum {self.tolerance_config.min_success_count}")
            return False
    ...
    def finalize_processing(self) -> None:
        ...
        if (
            ...
            raise PartialProcessingError(
                message=f"Processing completed with high failure rate: {summary['failure_rate']:.2%}",

    ...
    try:
        ...
        if memory_limit_mb is not None:
            ...
            if not suitable_sizes:
                recommendations.update({
                    "recommended_size": "small",
                    "reason": f"Memory limit {memory_limit_mb}MB too restrictive, using smallest available",
                    "warnings": [f"Recommended model requires {MODEL_CHARACTERISTICS['small']['memory_mb']}MB"]
                })
            else:

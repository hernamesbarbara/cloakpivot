...
class EntityNormalizer:
    ...
    def normalize_entities(self, entities: List[EntityDetectionResult]) -> NormalizationResult:
        ...
        logger.info(f"Normalization complete: {len(result.normalized_entities)} entities, "
    ...
    def _resolve_group_conflicts(self, group: EntityGroup, group_index: int) -> EntityGroup:
        ...
        logger.debug(f"Resolving conflicts in group {group_index} with {len(group.entities)} entities")
    ...
    def _resolve_by_confidence(self, entities: List[EntityDetectionResult]) -> List[EntityDetectionResult]:
    ...
    def _resolve_by_length(self, entities: List[EntityDetectionResult]) -> List[EntityDetectionResult]:
    ...
    def _resolve_by_specificity(self, entities: List[EntityDetectionResult]) -> List[EntityDetectionResult]:
        """Resolve conflicts by preferring more specific entity types."""
        # Sort by priority (lower number = higher priority)
        sorted_entities = sorted(
            entities, 
            key=lambda e: (self.config.get_entity_priority(e.entity_type).value, -e.confidence)
        )
    ...
    def _resolve_by_position(self, entities: List[EntityDetectionResult]) -> List[EntityDetectionResult]:
    ...
    def _resolve_by_merging(self, entities: List[EntityDetectionResult]) -> List[EntityDetectionResult]:
    ...
    def _merge_entity_group(self, entities: List[EntityDetectionResult], 
                          entity_type: str) -> EntityDetectionResult:
        ...
        logger.debug(f"Merged {len(entities)} {entity_type} entities into single entity "
    ...
    def _remove_overlaps(self, entities: List[EntityDetectionResult]) -> List[EntityDetectionResult]:
    ...
    def validate_normalization(self, original: List[EntityDetectionResult], 
                             normalized: List[EntityDetectionResult]) -> Dict[str, Any]:

...
class AnalyzerConfig:
    ...
    def _validate_nlp_engine(self) -> None:
        ...
        if self.nlp_engine_name not in valid_engines:
            raise ValueError(f"nlp_engine_name must be one of {valid_engines}, got '{self.nlp_engine_name}'")
...
class RecognizerRegistry:
    ...
    DEFAULT_RECOGNIZERS = {
        ...
        "PERSON", "URL", "IP_ADDRESS", "DATE_TIME", "LOCATION"
    }
    ...
    def __init__(self, enabled_recognizers: Optional[List[str]] = None):
        ...
        self._enabled: Set[str] = (
            set(enabled_recognizers) if enabled_recognizers is not None 
            else self.DEFAULT_RECOGNIZERS.copy()
        )
        self._custom_recognizers: Dict[str, Any] = {}
        
        logger.info(f"Initialized recognizer registry with {len(self._enabled)} enabled recognizers")
...
class AnalyzerEngineWrapper:
    ...
    def __init__(self, config: Optional[AnalyzerConfig] = None):
        ...
        logger.info(f"Created analyzer wrapper with language='{self.config.language}', "
    ...
    def analyze_text(self, text: str, entities: Optional[List[str]] = None, 
                    min_confidence: Optional[float] = None) -> List[EntityDetectionResult]:
        ...
        analyze_entities = entities if entities is not None else self.registry.get_enabled_recognizers()
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Test basic analysis
            test_results = self.analyze_text("Test text with email test@example.com", min_confidence=0.1)
        ...
        if self.config.min_confidence > 0.9:
            diagnostics["warnings"].append("Very high confidence threshold may miss valid entities")

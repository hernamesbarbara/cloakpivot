"""Batch processing engine for multi-document operations with progress reporting and error isolation."""
...
from .error_handling import ErrorCollector, ErrorRecord, FailureToleranceConfig, PartialFailureManager
...
class BatchProgressCallback(Protocol):
    ...
    def on_file_error(self, file_item: BatchFileItem, error: str, current_index: int) -> None:
...
class BatchProcessor:
    ...
    def __init__(
        ...
        if config.max_memory_mb:
            try:
                ...
                logger.warning("Memory monitoring not available - disabling memory limits")
    ...
    def discover_files(self) -> List[BatchFileItem]:
        ...
        if self.config.max_files_per_batch and len(discovered_files) > self.config.max_files_per_batch:
            logger.warning(
    ...
    def _calculate_output_path(self, input_path: Path) -> Optional[Path]:
        ...
        if self.config.operation_type in [BatchOperationType.MASK, BatchOperationType.UNMASK]:
    ...
    def process_batch(self) -> BatchResult:
        ...
        end_time = time.time()
        successful_files = [f for f in processed_files if f.status == BatchStatus.COMPLETED]
        ...
        result = BatchResult(
            config=self.config,
            status=BatchStatus.CANCELLED if self._is_cancelled else BatchStatus.COMPLETED,
            start_time=start_time,
            end_time=end_time,
    ...
    def _filter_existing_files(self, files: List[BatchFileItem]) -> List[BatchFileItem]:
        ...
        for file_item in files:
            ...
            if file_item.cloakmap_path and file_item.cloakmap_path.exists():
                logger.warning(f"CloakMap file exists, skipping: {file_item.cloakmap_path}")
                file_item.status = BatchStatus.FAILED
    ...
    def _process_files_parallel(self, files: List[BatchFileItem]) -> List[BatchFileItem]:
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    if result.status == BatchStatus.COMPLETED:
                        ...
                        self.progress_callback.on_file_error(result, result.error or "Unknown error", index)
                
                except Exception as e:
                    logger.error(f"Unexpected error processing {file_item.file_path}: {e}")
                    file_item.status = BatchStatus.FAILED
    ...
    def _process_single_file(self, file_item: BatchFileItem, index: int) -> BatchFileItem:
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                with self.profiler.measure_operation(f"batch.{self.config.operation_type.value}_file"):
                    ...
                    if self.config.operation_type == BatchOperationType.MASK:
                        ...
                        raise ValueError(f"Unsupported operation type: {self.config.operation_type}")
                ...
                error_msg = str(e)
                logger.warning(f"Attempt {attempt + 1} failed for {file_item.file_path}: {error_msg}")

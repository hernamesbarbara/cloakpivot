...
class DocumentProcessor:
    ...
    def load_document(
        ...
        **kwargs: Any
        ...
        try:
            ...
            document = load_document(file_path, **kwargs)
            ...
            logger.debug(f"Document contains: {len(document.texts)} text items, "
                        f"{len(document.tables)} tables, "
                        f"{len(document.pictures)} pictures")
            
            return document
            
        except (FileAccessError, UnsupportedFormatError, ValidationError, TransformationError):
    ...
    def _validate_document_structure(self, document: DoclingDocument) -> None:
        ...
        has_text_content = (
            len(document.texts) > 0 or 
            len(document.tables) > 0 or 
            len(document.key_value_items) > 0
        )

...
class AnchorMapper:
    ...
    def create_anchors_from_detections(
        ...
        Examples:
            ...
            ... )
        """
        logger.info(f"Creating anchors from {len(detections)} detections across {len(segments)} segments")
        ...
        for detection in detections:
            ...
            if not segment:
                logger.warning(f"No segment found for detection at global position {detection.start}")
                continue
            ...
            anchor = AnchorEntry.create_from_detection(
                ...
                metadata={
                    ...
                    "presidio_analysis_explanation": detection.analysis_explanation.__dict__ if detection.analysis_explanation else None
                }
            )
            
            anchors.append(anchor)
            logger.debug(f"Created anchor for {detection.entity_type} at {node_ref.node_id}[{node_ref.start_pos}:{node_ref.end_pos}]")
    ...
    def map_global_to_node_position(
        ...
        if global_end > containing_segment.end_offset:
            logger.warning(f"Position span [{global_start}, {global_end}] crosses segment boundaries")
            return None
        ...
        # Find segment index
        segment_index = next(
            (i for i, seg in enumerate(segments) if seg == containing_segment),
            -1
        )
    ...
    def map_node_to_global_position(
        ...
        matching_segment = next(
            (seg for seg in segments if seg.node_id == node_id),
            None
        )
        ...
        if start_pos < 0 or end_pos > len(matching_segment.text):
            logger.warning(f"Positions [{start_pos}, {end_pos}] outside segment bounds for {node_id}")
            return None
            ...
            for anchor in node_anchors:
                ...
                if not overlaps:
                    ...
                    if resolved_anchor:
                        ...
                        non_overlapping = [a for a in non_overlapping if not a.overlaps_with(resolved_anchor)]
    ...
    def _create_node_reference(
        ...
        try:
            ...
            # Find segment index
            segment_index = next(
                (i for i, seg in enumerate(all_segments) if seg == segment),
                -1
            )
    ...
    def _resolve_overlap_conflict(
        self,
        new_anchor: AnchorEntry,
        existing_anchors: List[AnchorEntry]
        ...
        logger.debug(f"Resolved conflict: chose {best_anchor.entity_type} "

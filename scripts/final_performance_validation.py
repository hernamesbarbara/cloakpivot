...
from cloakpivot.loaders import get_presidio_analyzer, get_detection_pipeline, get_cache_info, clear_all_caches
...
class PerformanceTarget:
    ...
    def target_met(self) -> Optional[bool]:
        ...
        if "reduction" in self.description.lower():
            return self.improvement_pct and self.improvement_pct >= self.target_value
    ...
    def generate_validation_report(self, validation_results: Dict[str, Any]) -> str:
        ...
        for name, result in validation_results.items():
            ...
            if current_val is not None:
                report_lines.append(f"- **Current Value**: {current_val:.2f} {target.target_unit.replace('% reduction', '').replace('% hit rate', '%').replace('regressions', '').strip()}")
        ...
        if not unmet_targets:
            report_lines.append("🎉 All performance targets have been achieved! The optimization effort has been successful.")
            report_lines.append("")
            ...
            for target_name in unmet_targets:
                ...
                report_lines.append(f"- **{target.description}**: Review implementation and consider additional optimization")

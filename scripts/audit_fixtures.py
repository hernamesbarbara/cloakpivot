...
class FixtureInfo:
    ...
    def __init__(self, name: str, scope: str, file_path: Path) -> None:
        self.name = name
        self.scope = scope
...
class FixtureAnalyzer:
    ...
    def _analyze_fixture_properties(
        self, func_node: ast.FunctionDef, fixture_info: FixtureInfo, content: str
    ) -> None:
        ...
        if any(
            ...
            for word in ["expensive", "slow", "heavy", "initialization"]
        ):
            fixture_info.is_expensive = True

        if any(
            word in docstring.lower()
            for word in ["isolation", "reset", "clean", "temporary"]
        ):
            fixture_info.is_stateful = True

    def _analyze_fixture_usage(self, file_path: Path) -> None:
...
def generate_optimization_report(fixtures: dict[str, FixtureInfo]) -> str:
    ...
    for fixture in fixtures.values():
        ...
        if recommended_scope and recommended_scope != current_scope:
            ...
            fixture.is_expensive
            and not fixture.is_stateful
            and current_scope == "function"
        ):
            session_candidates.append(fixture)
    ...
    if session_candidates:
        ...
        for fixture in session_candidates:
            ...
            report.append(
                f"- **{fixture.name}** (current: {fixture.scope}, used by {usage_count} files)"
            )
            if fixture.is_expensive:
                report.append("  - ✅ Expensive to create")
            if len(fixture.usages) > 1:
                report.append("  - ✅ Reused across multiple files")
            if not fixture.is_stateful:
                report.append("  - ✅ Stateless, safe for session scope")
            report.append("")
    ...
    report.extend(
        [
            ...
            "- Converting to session scope could reduce this by ~80-90%",
            "",
        ]
    )
...
def main() -> int:
    ...
    session_candidates = [
        ...
        if f.scope != "session"
        and (f.is_expensive or len(f.usages) > 2)
        and not f.is_stateful
    ]

    print(f"Session scope candidates: {len(session_candidates)}")
    for fixture in session_candidates[:5]:  # Top 5
        print(
            f"  - {fixture.name} ({fixture.scope} -> session, {len(fixture.usages)} usages)"
        )
